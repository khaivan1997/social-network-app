...
public class Activity_Comment extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        this.viewModel.getMode().observe(this, new Observer<Integer>() {
            @Override
            public void onChanged(Integer integer) {
                if(integer != -1)
                    Toast.makeText(Activity_Comment.this,"Request fails", Toast.LENGTH_LONG).show();
            }
        });

        this.comment = findViewById(R.id.comment_field);
        this.button = findViewById(R.id.post_comment);
